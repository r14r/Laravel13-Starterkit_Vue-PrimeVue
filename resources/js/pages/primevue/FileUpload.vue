...
function onSelect(event: FileUploadSelectEvent) {
    uploadedFiles.value = event.files as File[];
    totalSize.value = uploadedFiles.value.reduce((acc, f) => acc + f.size, 0);
}

function onRemove(event: FileUploadRemoveEvent) {
    totalSize.value -= (event.file as File).size;
}
...
<template>
    ...
    <div class="p-6 space-y-8">
        ...
        <section class="space-y-4">
            ...
            <p v-if="uploadedFiles.length" class="text-sm text-muted-foreground">
                {{ uploadedFiles.length }} file(s) selected — Total: {{ formatSize(totalSize) }}
            </p>
        </section>
